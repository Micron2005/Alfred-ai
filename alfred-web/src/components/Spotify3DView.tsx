...
export function Spotify3DView({ onBack }: Spotify3DViewProps) {
  ...
  // Track the actual scopes the linked account has so we can prompt
  // a re-link when the user's existing token was minted before we
  // added playlist-read-private + user-library-read. Without those
  // scopes, the new browse/search endpoints return Spotify's
  // misleading "400 Invalid limit" error. Detected by simply
  // inspecting whether the playlists fetch failed AND search
  // failed — both fail iff scopes are stale.
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Stale-scope detection — Spotify returns "Invalid limit"
        // when an endpoint is forbidden for the dev-mode app or the
        // token lacks the right scope. Real "limit too big" errors
        // are impossible from our code (we always clamp).
        if (/invalid\s+limit|insufficient\s+scope|403|401/i.test(message)) {
          setScopeStale(true);
        }
      }
    })();
      ...
      >
        ...
        >
          ...
          {!status && !statusError ? (
            ...
            <>
              {/* Permanently-visible reconnect strip. Spotify periodically
                  changes scopes / deprecates endpoints, so the user
                  may need to re-link without us being able to detect
                  it ahead of time. Showing this button always — even
                  when things look healthy — gives them a clear escape
                  hatch the moment something goes wrong. The yellow
                  variant below appears in addition when we KNOW the
                  scopes are stale. */}
              <div
                style={{
                  display: "flex",
              ...
              >
                <span>Trouble with playlists / search?</span>
                <button
                  type="button"
                  data-testid="spotify-3d-relink-always"
                  ...
                  style={{ fontSize: 10, letterSpacing: 1.5, padding: "4px 10px" }}
                >
                  🔄 RECONNECT
                </button>
              </div>
              {scopeStale ? (
                <div
                  data-testid="spotify-3d-scope-stale"
                  style={{
                    fontSize: 11,
                    lineHeight: 1.5,
                    color: "var(--accent)",
                ...
                >
                  <div style={{ fontWeight: 600, letterSpacing: 0.5 }}>
                    SPOTIFY ACCESS IS STALE
                  </div>
                  <div style={{ color: "var(--muted)", fontSize: 10 }}>
                    Your existing OAuth token was minted before we added
                    playlist + library scopes. Disconnect and reconnect to
                    grant the broader access — your playback controls will
                    keep working as before, plus playlists and search will
                    light up.
                  </div>
                  <button
                    type="button"
                    data-testid="spotify-3d-relink"
                    className="hud-button hud-button--primary"
                    disabled={busy}
                    onClick={async () => {
                      setBusy(true);
                  ...
                  >
                    {busy ? "RELINKING…" : "🔄 RECONNECT SPOTIFY"}
                  </button>
                </div>
              ) : null}
